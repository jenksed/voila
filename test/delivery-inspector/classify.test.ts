...
test("every category is reachable and each classification names its evidence", () => {
  const expectations: [string, string][] = [
    ...
    [".voila/project.json", "project_state"],
...
test("rule order resolves the paths that could match more than one rule", () => {
  // Generated Voila views must not be read as documentation or as canonical state.
  assert.equal(classifyPath(".voila/status/STATUS.md").category, "generated");
  assert.equal(classifyPath(".voila/project.json").category, "project_state");
