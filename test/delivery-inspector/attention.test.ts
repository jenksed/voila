...
test("a generated NewFang view changing without canonical state is flagged", () => {
  const drifted = detect([file(".newfang/status/STATUS.md")]);
  const item = find(drifted, "generated_view_without_state_change");
  assert.match(item.reason, /derived output/);

  const consistent = detect([file(".newfang/status/STATUS.md"), file(".newfang/project.json")]);
