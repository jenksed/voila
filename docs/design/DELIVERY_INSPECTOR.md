...
- touch the network;
- read or write `.newfang/` canonical state, or import anything from `src/domain/` or `src/state/`;
...
|----------|------------------|
| `generated` | build/coverage output directory, `.newfang/**/*.md` view, `.generated.`, `.min.js`, `.map` |
| `project_state` | non-view files under `.newfang/` |
...
before `documentation` (a generated NewFang view is neither state nor docs); `ci` before `configuration`;
...
| `generated_view_without_state_change` | a `.newfang/` view changed with no canonical state change |
...
This mirrors the existing NewFang convention in
...
| Persisting an inspection | The result is a snapshot with no timestamp, like an orientation artifact. 5B decides whether to persist it, under what id, and how staleness is evaluated (HEAD movement is the obvious signal). The inspector must not write `.newfang/`. |
