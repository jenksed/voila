...
Start Voila as **project-local Pi extensions** (Option 1), with a **repo-visible Markdown/JSONL
...
MVP needs it. Package as an `@voila` pi package (Option 2) once sharing matters. Keep the SDK and a
...
Installed Pi CLL (`pi`)  ─ harness, run loop, TUI, model registry, session tree, events, tools
        │
        ▼  loads
Voila (project-local extensions in .pi/extensions/, later an @voila pi package)
        │
...
.voila/  project.json (authoritative snapshot) + events.jsonl (append-only) + receipts/ + views/PROJECT_STATUS.md (generated)
        ▲
...
- **Pi (unchanged, installed):** run loop, TUI primitives, model/provider registry, session tree,
  compaction, events, built-in tools, trust. Voila does not fork or vendor Pi.
- **Voila extensions:** the tools/commands/UI/events above. Each extension owns one concern
  (intake, ledger, claims, gates, home-view, delegation, delivery) and is independently testable.
- **Voila roles:** responsibility definitions expressed as skills/prompt templates and (for
  delegation) agent frontmatter `.md` files. Roles are model-independent; model choice is a routing
  concern layered on top.
- **Voila state:** the `.voila/` directory (authoritative `project.json` + append-only
...
- **Thin adapter rule:** `.pi/extensions/voila.ts` is a thin Pi adapter; production logic lives in
  modular `src/` (see [ADR-0007](../decisions/0007-thin-adapter-modular-src.md)).

## Authoritative state owner (decisive, locked)

The **`.voila/` directory is authoritative**, not Pi session entries or model context, per
[ADR-0003](../decisions/0003-authoritative-state-is-human-readable-ledger.md):

- `.voila/project.json` — authoritative current-state snapshot.
- `.voila/events.jsonl` — append-only history (never authoritative current state).
- `.voila/receipts/` — immutable verification evidence (later packets).
- `.voila/views/PROJECT_STATUS.md` — generated, human-readable projection (marked generated).
...
bootstrap; it is not the runtime canonical store. The runtime canonical store is `.voila/`.

## Extension / package boundaries

- **MVP:** project-local `.pi/extensions/` in the Voila repo (and, for dogfooding, installed into a
  target project's `.pi/`). No package manifest yet.
- **Thin adapter:** `.pi/extensions/voila.ts` contains only Pi loading/composition; all domain and
...
- Voila runs **inside** the Pi process as loaded extensions. No separate Voila process in the
  MVP.
...
- **Pinned foundation (Packet 1):** `@earendil-works/pi-coding-agent@0.82.0` (engines
  ...
  via Pi. Voila currently imports only the coding-agent package (types + SDK for the integration
...
- **Dev (pinned):** `typescript@7.0.2`, `@types/node@22.20.1`, `prettier@3.9.6`, plus Node's built-in
  ...
  dependency yet (state validation is hand-rolled); it is adopted when Voila registers LLM tool
  schemas.
...
- **Reconciliation tests**: repo-vs-session divergence resolves deterministically (repo wins, with
  attribution).
- Meet or exceed the reference setup's test discipline; a completion claim for any Voila feature
...
- Keep Voila's authoritative state in repo files so a Pi upgrade cannot invalidate project truth.
...
- The extension and SDK surfaces Voila uses are **documented for and verified against the pinned
...
- The user has, or will configure, at least one working model provider for Pi via `/login` (a manual
  step Voila never performs on the user's behalf). Not required for Packet 1's non-model paths.
...
| `.voila/` canonical state model is correct (locked, ADR-0003) | High |
...
- **Locked now:** extensions-first start (ADR-0002); `.voila/` canonical state model (ADR-0003);
