...
| Item | Value |
|------|-------|
| Worktree | `/Users/jenksed/Projects/newfang-delivery-inspector` (git worktree) |
...
`.pi/**`, or `.newfang/**`. No `test/fixtures/` directory was added; the in-memory filesystem made one
unnecessary.
...
- No reading or writing of `.newfang/` canonical state; no schema change; no migration.
