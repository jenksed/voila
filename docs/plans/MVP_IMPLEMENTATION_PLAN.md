...
[`newfang-approval-bundles`](SELF_HOSTING_ACCEPTANCE_PROJECT.md) as the first self-hosted project.
...
- **Objective**: Install Pi, prove a NewFang extension loads and runs, and lock the extension/test
  scaffolding.
- **Tasks**:
  1. Install Pi (`@earendil-works/pi-coding-agent`, target `0.82.x`), project-scoped where possible;
     record the exact version.
  2. Create `.pi/extensions/newfang/` skeleton: one `/newfang status` command and a trivial home-view
     widget.
  3. Set up TypeScript + formatter + Node test runner; add a first passing unit test.
- **Dependencies**: none (Phase 0 done).
- **Acceptance gate**: `pi` loads the extension; `/newfang status` responds; the widget renders; the
  test passes.
- **Verification/receipt**: terminal transcript of `/newfang status`; `test` output saved.
...
- **Runnable artifact**: a loadable NewFang skeleton.

## Phase 2 — Durable ledger + backlog (inspectable)

- **Objective**: Canonical `.newfang/` project state (ADR-0003) that survives restart. (Note: the
...
- **Acceptance gate**: create items, restart Pi, reconstruct identically from `.newfang/`; the
...
- **Tasks**:
  ...
  3. Next-action logic surfaced via `/newfang status` and the home view.
- **Dependencies**: Phase 2.
- **Acceptance gate**: given a sample `PLAN.md` in a target repo, NewFang produces a correct brief +
...
- **Objective**: Make evidence-before-completion real at the level NewFang can guarantee — its
...
- **Tasks**:
  ...
  2. Verification receipt (command via `pi.exec`, environment, versions, result, output path) stored
     under `.newfang/receipts/` and linked to a claim.
  3. A `newfang_complete_work_item` state transition that is **rejected** unless the item's required
     claim has a passing receipt. The guarantee is about `project.json`: an item is not recorded
     complete unless the gate passes. NewFang cannot prevent an LLM from writing "done" in prose.
...
- **Acceptance gate**: NewFang suggests a well-scoped commit and emits a delivery summary tied to
...
- **Acceptance gate**: all eight capabilities demonstrated with receipts; if any fail, iterate the
  relevant phase.
- **Verification/receipt**: a checklist with linked receipts in `.newfang/` and the ledger.
- **Output**: an explicit go/no-go for self-hosting.

## Phase 8 — Build `newfang-approval-bundles` under NewFang (self-hosted delivery)

- **Objective**: Use NewFang to design, build, verify, and deliver the approval subsystem.
...
- **Acceptance gate**: the project's completion criteria met; control-transfer from bootstrap to
  NewFang recorded in the ledger.
- **Verification/receipt**: the approval subsystem's own tests + receipts; delivery summary/PR.
- **Runnable artifact**: NewFang built a real extension *for itself*, verified by evidence.

## Cross-phase acceptance gates (apply to every phase)

- Every NewFang tool/behavior has unit tests; a completion claim cites passing tests.
